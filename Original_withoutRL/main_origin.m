...
config.ampli_u = deg2rad(30);
config.omega_u = 3;
config.delta_u = deg2rad(60);
...
% Center Tracking
[T1,X1]=ode45(@(t,x) SnakeRobot_CentreTrack_model(t,x,config), tspan, x0); 
...
%Px = X1(:, config.N+1) * 100;  % Centre of mass on x-axis in cm
%Py = X1(:, config.N+2) * 100;  % Centre of mass on y-axis in cm

% Extract the center of mass velocity of the snake robot
PxDot = X1(:, 2*config.N+3);  
PyDot = X1(:, 2*config.N+4); 
%PxDot = X1(:, 2*config.N+3) * 100;  
%PyDot = X1(:, 2*config.N+4) * 100; 
