%% Define the snake robot environment class
classdef SnakeRobot_env_DDPG < rl.env.MATLABEnvironment
    properties
    ...
    methods
        function this = SnakeRobot_env_DDPG(config, agent)
        ...
        function [NextObservation, Reward, IsDone] = step(this,~)
            % Use DDPG agent to get corrective action for theta and thetaDot
            action = getAction(this.agent, this.state);
            % Scale the output from tanh layer
            if iscell(action)
            ...
            [~, x] = ode45(@(t, x) SnakeRobot_CentreTrack_DDPG_Dynamics(t, x, this.config, u_theta_ref, u_theta_refDot), [0, 0.04], this.state);
